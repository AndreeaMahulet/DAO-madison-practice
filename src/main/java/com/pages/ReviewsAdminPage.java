...
public class ReviewsAdminPage extends BasePage{
    public ReviewsAdminPage(WebDriver driver) {
        super(driver);
    }
    @FindBy(css = "#reviwGrid_table > tbody > tr:nth-child(1)")
    private List<WebElementFacade> reviewContainer;
    @FindBy(css = "#status_id")
    private WebElement statusReviewDropDownList;
    ...
    public WebElement approveReview(String text) {
        Select selectedOption=new Select(statusReviewDropDownList);
        List<WebElement> statusReview = selectedOption.getOptions();
        for (WebElement approve : statusReview) {
